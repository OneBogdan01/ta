...
void internal::init_triangle_pipeline()
{
  ...
  if (!vkutil::load_shader_module(
          io::GetPath("shaders/colored_triangle.frag.spv").c_str(), _device,
          &triangleFragShader))
  {
  ...
  if (!vkutil::load_shader_module(
          io::GetPath("shaders/colored_triangle.vert.spv").c_str(), _device,
...
void internal::init_background_pipelines()
{
  ...
  if (!vkutil::load_shader_module(
          io::GetPath("shaders/gradient_color.comp.spv").c_str(), _device,
          &gradientShader))
  {
  ...
  if (!vkutil::load_shader_module(io::GetPath("shaders/sky.comp.spv").c_str(),
                                  _device, &skyShader))
  {
    fmt::print("Error when building the compute shader \n");
  }
